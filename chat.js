...
// This runs on Cloudflare's server, NOT in the visitor's browser — so the API key
// set in your Cloudflare dashboard (GROQ_API_KEY) is never visible to website visitors.
//
// It receives { gameKey, messages } from script.js, adds the right game's knowledge
// base as a system prompt, calls Groq's free-tier API, and sends back the answer.
...
${knowledge}`;
    ...
    // Groq's API is OpenAI-compatible. Free tier — see deployment instructions
    // for how to get a key and set it as the GROQ_API_KEY secret in Cloudflare.
